...
export const checkoutRouter = createTRPCRouter({
  verify: protectedProcedure.mutation(async ({ ctx }) => {
    const user = await ctx.db.findByID({
      ...
      depth: 0, //use.tenants.[0].tenant is going to be string [tenant id]
    });
    if (!user) {
      throw new TRPCError({ code: "NOT_FOUND", message: "User not found" });
    }
    const tenantId = user.tenants?.[0]?.tenant as string; // this is because depth 0
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const lineItems: Stripe.Checkout.SessionCreateParams.LineItem[] =
        products.docs.map((product) => ({
          quantity: 1,
          price_data: {
            unit_amount: product.price * 100, //stripe handles pricess in cents
            currency: "usd",
            product_data: {
              name: product.name,
              metadata: {
      ...
      //subdomain redirect checkout session
